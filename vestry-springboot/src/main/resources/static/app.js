...
function apiFetch(url, options = {}) {
  ...
  // No agregar Content-Type si es FormData (el browser lo pone con boundary)
...
function switchView(view) {
  ...
  // Resetear toolbar al cambiar de vista
  const viewEl = $(`view-${view}`);
  const toolbar = viewEl?.querySelector('.content-toolbar');
  if (toolbar) toolbar.classList.remove('scrolled');
...
// Frosted glass toolbar al hacer scroll
document.querySelectorAll('.view').forEach(viewEl => {
  viewEl.addEventListener('scroll', () => {
    const toolbar = viewEl.querySelector('.content-toolbar');
    if (!toolbar) return;
    toolbar.classList.toggle('scrolled', viewEl.scrollTop > 8);
  }, { passive: true });
});
...
async function loadGarments() {
  try {
    const res = await apiFetch(`${API}/garments?userId=${AppState.currentUser.id}`);
...
async function createGarment() {
  ...
  try {
    const res     = await apiFetch(`${API}/garments`, { method: 'POST', body: fd });
...
async function updateGarment(id) {
  ...
  try {
    const res     = await apiFetch(`${API}/garments/${id}`, { method: 'PATCH', body: fd });
    const garment = await res.json();
...
async function loadOutfits() {
  try {
    const res = await apiFetch(`${API}/outfits?userId=${AppState.currentUser.id}`);
...
async function saveOutfit() {
  ...
  try {
    const res    = await apiFetch(`${API}/outfits`, { method: 'POST', body: fd });
...
$('btn-clear-cal-day').addEventListener('click', async () => {
  ...
  try {
    await apiFetch(`${API}/calendar/${entry.id}`, { method: 'DELETE' });
...
async function loadAnalytics() {
  try {
    const res  = await apiFetch(`${API}/analytics?userId=${AppState.currentUser.id}`);
    const data = await res.json();
